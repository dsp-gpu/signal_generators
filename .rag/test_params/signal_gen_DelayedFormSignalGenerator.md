...
parser_version: 2
synonyms_ru: ['Генератор с задержкой', 'Фарроу-генератор', 'Дробная задержка', 'Сигнал с сдвигом']
synonyms_en: ['Fractional delay generator', 'Farrow generator', 'Delayed signal', 'Form signal']
...
**ЧТО**: GPU-генератор сигналов с дробной задержкой на основе Farrow-интерполяции (Lagrange 48×5).

**ЗАЧЕМ**: Решает задачу применения переменных задержек per-antenna с высокой точностью для радиолокационной обработки.

**КАК**: Использует LchFarrow для дробной задержки, кэширует параметры, поддерживает batch-генерацию для множества антенн. Оптимизировано для ROCm/HIP.
